...
results_nee <- haven::read_dta(paste0(dir_data, "results_nee.dta"))
results_nee <- results_nee %>% mutate(est = est * 100, cil = cil *100, ciu = ciu * 100)

graph <- function(ngrado, nyear){
  # Base graph
  (base_nee <- ggplot(data = results_nee %>% 
                        filter(agno == nyear & grado == ngrado & mod == 3 & gpa == 1)) + 
     geom_line(aes( y = est, x = t,  color = "LP (D1)")) + 
     geom_point(aes( y = est, x = t, color = "LP (D1)")) + 
     geom_line(data = results_nee %>% 
                 filter(agno == nyear & grado == ngrado & mod == 3 & gpa == 2), aes( y = est, x = t, color = "LP (D2)"))  + 
     geom_point(data = results_nee %>% 
                  filter(agno == nyear & grado == ngrado & mod == 3 & gpa == 2), aes( y = est, x = t, color = "LP (D2)")) + theme_classic() + 
     geom_hline(yintercept = 0) + 
     geom_hline(yintercept = -4, color = "gray" ) + 
     geom_hline(yintercept = 4, color = "gray") + 
     geom_vline(xintercept = 0, color = "red", linetype = "dashed") +
     scale_x_continuous( labels = scales::number_format(accuracy = 1), breaks = -5:5 ) +
     scale_y_continuous( breaks = seq(from = -8, to = 8, by = 4), lim = c(-8, 8), labels = dollar_format(suffix = "%", prefix = "")) + 
     xlab("T") + ylab("Event study estimate")+
     theme(legend.title=element_blank())) + 
    scale_color_manual(values=c("green", "darkgreen")) 
  
  
  # Graph With and without  NEE 
  CI_nee <- base_nee + 
    geom_line(data = results_nee %>% 
                filter(agno == nyear & grado == ngrado & mod == 30 & gpa == 1), aes( y = est, x = t, color = "LP (D1) (w/o) exempted"), linetype = "dashed") + 
    geom_point(data = results_nee %>% 
                 filter(agno == nyear & grado == ngrado & mod == 30 & gpa == 1), aes( y = est, x = t, color = "LP (D1) (w/o) exempted")) + 
    geom_line(data = results_nee %>% 
                filter(agno == nyear & grado == ngrado & mod == 30 & gpa == 2), aes( y = est, x = t, color = "LP (D2) (w/o) exempted"), linetype = "dashed") + 
    geom_point(data = results_nee %>% 
                 filter(agno == nyear & grado == ngrado & mod == 30 & gpa == 2), aes( y = est, x = t, color = "LP (D2) (w/o) exempted")) + 
    scale_color_manual(" ", values = c("blue","#07EAF2", "#E707F2", "#F20727")) +
    scale_linetype_manual(values = c("dashed", "dashed", "twodash", "twodash")) 
  
  if (ngrado == 4) {
    CI_nee <- CI_nee + geom_vline(xintercept = 1, color = "red", linetype = "dashed") 
  }
  return(CI_nee)
}  

g1 <- graph(2,2014) + ggtitle("2nd grade \n")+ 
...
g2 <- graph(4,2014) + ggtitle("4th grade \n") + 
  theme(plot.title = element_text(hjust = 0.5)) + 
  theme( axis.text.y = element_text(size = 14),
         axis.text.x = element_text(size = 14), 
         axis.title=element_text(size = 14,face="bold"), 
