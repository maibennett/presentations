...
```{r prediction_all_boxplot, fig.height=5.5, fig.width=10, fig.align='center', dev='svg', echo=FALSE, warning=FALSE, message=FALSE}
dir_data2 = "C:/Users/mc72574/Dropbox/UT/UT Research/Asistencia/"

df_final= read.csv(paste0(dir_data2,"data/working/boxplot_data_xgboost2017_v7.csv"))

ggplot(data = df_final, aes(x = GPA_pos, y = mean_outcome*100, fill = factor(type), color = factor(type))) + 
  geom_point(pch = 21, position = position_dodge(width = 1), alpha=.6, size = 4) +
  geom_errorbar(aes(x=GPA_pos, ymin=ci_lower*100, ymax=ci_upper*100, color = factor(type)), 
...
- AUC for non-tested grades: 0.798
