...
.left[.small[Seminario Ingeniería UC<br>August 8th, 2023]]]
...
<br>
<br>
.center[.box-2Trans[Causal Inference]]
...
<br>
<br>
.center[.box-3Trans[Prediction (ML)]]

]
--

.center[
![:scale 30%](https://raw.githubusercontent.com/maibennett/presentations/main/content/presentations/Attendance/PRC_20220426/images/arrows.png)]
.box-5Trans[Evidence-Based Policy]
...
```{r event_study_nee_plot, fig.height=4.6, fig.width=8, fig.align='center', dev='svg', echo=FALSE, warning=FALSE, message=FALSE}
library(kableExtra)
...
graph <- function(ngrado, nyear){
  ...
  base_nee <- ggplot(data = results_nee %>% 
     ...
     geom_point(aes( y = est, x = t, color = "LP (D1)", fill = "LP (D1)"), pch=21, size = 2, stroke=1.1) + 
     ...
     geom_point(data = results_nee %>% 
                  filter(agno == nyear & grado == ngrado & mod == 3 & gpa == 2), aes( y = est, x = t, color = "LP (D2)", fill = "LP (D2)"), pch=21, size = 2, stroke=1.1) + theme_classic() + 
     geom_hline(yintercept = 0) + 
     geom_hline(yintercept = -4, color = "gray" ) + 
     geom_hline(yintercept = 4, color = "gray") + 
     geom_vline(xintercept = 0, color = text_color, linetype = "dashed") +
     scale_x_continuous( labels = scales::number_format(accuracy = 1), breaks = -5:5 ) +
    ...
    #scale_color_manual(values = c(alpha(cols[2],0.3),alpha(cols[3],0.3)))
  ...
  CI_nee <- base_nee + 
    ...
    geom_point(data = results_nee %>% 
                 filter(agno == nyear & grado == ngrado & mod == 30 & gpa == 1), aes( y = est, x = t, color = "LP (D1) (w/o) exempted", fill = "LP (D1) (w/o) exempted"), pch=22, size = 2, stroke=1.1) + 
    geom_line(data = results_nee %>% 
                filter(agno == nyear & grado == ngrado & mod == 30 & gpa == 2), aes( y = est, x = t, color = "LP (D2) (w/o) exempted"), linetype = "dashed") + 
    geom_point(data = results_nee %>% 
                 filter(agno == nyear & grado == ngrado & mod == 30 & gpa == 2), aes( y = est, x = t, color = "LP (D2) (w/o) exempted", fill = "LP (D2) (w/o) exempted"), pch=22, size = 2, stroke=1.1) + 
    scale_color_manual(name = "Estimate", values = c(cols[2],cols[3], cols[4], cols[5])) +
    scale_fill_manual(name = "Estimate", values = c(alpha(cols[2],0.3),alpha(cols[3],0.3), alpha(cols[4],0.3),
                                                    alpha(cols[5],0.3))) +
    scale_linetype_manual(name = "Estimate", values = c("dashed", "dashed", "twodash", "twodash")) 
  
  if (ngrado == 4) {
    CI_nee <- CI_nee + geom_vline(xintercept = 1, color = text_color, linetype = "dashed") 
  }
  return(CI_nee)
}  

g2 <- graph(4,2014) + xlab("Days since test") + ylab("Event study estimate")+
    theme_ipsum_fsc() +
    theme(plot.margin=unit(c(0.5,0,1.5,0),"cm"),
          panel.grid.major.x = element_blank(),
          panel.grid.minor.x = element_blank(),
          #panel.grid.major.y = element_blank(),
          panel.grid.minor.y = element_blank(),
...
- Use **.coolblue[GPBoost]** with panel data for **.coolblue[attendance prediction]** 
  
  - Combines Gaussian Processes and Gradient Boosting.
  
  - Model includes random effects for both subjects and time.
  
  - Predictive variables include day of the week, grade, GPA group, and sibling's attendance. 
...
df_plot <- read.csv("https://raw.githubusercontent.com/maibennett/presentations/main/content/presentations/Attendance/IngUC_20230810/data/plot_data_gpboost2017_updated_all_schoolsv5.csv")
...
g1 = ggplot(df_wide_test, aes(x = cluster, y = diff_4_D1)) + 
  ...
  xlab("Cluster") +
  ylab("Diff. obs - predicted") + ggtitle("D1 GPA") +
  scale_color_manual(values = c(cols[3],cols[2])) +
  scale_fill_manual(values = c(alpha(cols[3],0.4),alpha(cols[2],0.4))) +
  #ggplot2::annotate("text", x = 10, y = 6, label = "Adj. R2 = 0.48", size = 6, colour = "dark grey", hjust=0,family="Fira Sans Condensed") +
...
g2 = ggplot(df_wide_test, aes(x = cluster, y = diff_4_D2)) + 
        geom_boxplot(aes(fill = cluster, color = cluster)) + 
  xlab("Cluster") +
  ylab("Diff. obs - predicted") + ggtitle("D2 GPA") +
  scale_color_manual(values = c(cols[3],cols[2])) +
  scale_fill_manual(values = c(alpha(cols[3],0.4),alpha(cols[2],0.4))) +
  #ggplot2::annotate("text", x = 10, y = 6, label = "Adj. R2 = 0.48", size = 6, colour = "dark grey", hjust=0,family="Fira Sans Condensed") +
  theme_ipsum_fsc(plot_title_face = "bold",plot_title_size = 18) + #plain
...
g3 = ggplot(df_wide_test, aes(x = cluster, y = diff_4_D9)) + 
                geom_boxplot(aes(fill = cluster, color = cluster)) + 
  xlab("Cluster") +
  ylab("Diff. obs - predicted") + ggtitle("D9 GPA") +
  scale_color_manual(values = c(cols[3],cols[2])) +
  scale_fill_manual(values = c(alpha(cols[3],0.4),alpha(cols[2],0.4))) +
  #ggplot2::annotate("text", x = 10, y = 6, label = "Adj. R2 = 0.48", size = 6, colour = "dark grey", hjust=0,family="Fira Sans Condensed") +
  theme_ipsum_fsc(plot_title_face = "bold",plot_title_size = 18) + #plain
...
g4 = ggplot(df_wide_test, aes(x = cluster, y = diff_4_D10)) + 
                geom_boxplot(aes(fill = cluster, color = cluster)) + 
  xlab("Cluster") +
  ylab("Diff. obs - predicted") + ggtitle("D10 GPA") +
  scale_color_manual(values = c(cols[3],cols[2])) +
  scale_fill_manual(values = c(alpha(cols[3],0.4),alpha(cols[2],0.4))) +
  #ggplot2::annotate("text", x = 10, y = 6, label = "Adj. R2 = 0.48", size = 6, colour = "dark grey", hjust=0,family="Fira Sans Condensed") +
  theme_ipsum_fsc(plot_title_face = "bold",plot_title_size = 18) + #plain
...
```{r dist_score_cluster, fig.height=5.5, fig.width=8, fig.align='center', dev='svg', echo=FALSE, warning=FALSE, message=FALSE}

d_tab %>% filter(!is.na(cluster)) %>%
ggplot(aes(x = `Avg. SIMCE Math`, color = factor(cluster), fill = factor(cluster))) +
  geom_density() +
  scale_color_manual(name = "Cluster", labels = c("Increase Att","Lower att"),values = c(cols[2],cols[3])) +
  scale_fill_manual(name = "Cluster", labels = c("Increase Att","Lower att"),values = c(alpha(cols[2],0.2),alpha(cols[3],0.2))) +
  xlab("SIMCE Math") + ylab("Density") +
...
- The previous differences between types of schools **.coolblue[does not capture the true difference]** given non-representative attedance patterns.
...
- Two incentives working simultaneously:
  ...
  - Incentive for **.coolblue[lower-performing students not to attend]** the day of the test
  
  - Incentive for **.coolblue[higher-performing students to attend]** the day of the test
  
--
<br>
<br>
- We will focus on solving the **.coolblue[first one]**.
...
```{r imputation_diff1, fig.height=5.5, fig.width=8, fig.align='center', dev='svg', echo=FALSE, warning=FALSE, message=FALSE}
...
ggplot(data = impute2, aes(x = factor(imputation, levels = c("Impute All",
...
#                   yend = impute$mean[impute$cluster==2 & impute$imputation=="Impute Pred-Obs (Decile)"]), color = "grey") +
  ...
  scale_color_manual(name = "Cluster", labels = c("Increase Att","Lower att"),values = c(cols[2],cols[3])) +
  scale_fill_manual(name = "Cluster", labels = c("Increase Att","Lower att"),values = c(alpha(cols[2],0.8),alpha(cols[3],0.8))) +
  ...
  theme(axis.title.x = element_text(size=14),#margin = margin(t = 10, r = 0, b = 0, l = 0)),
        ...
        axis.title.y = element_text(size=14),#margin = margin(t = 0, r = 10, b = 0, l = 0)),
        axis.text.y = element_text(size=12),legend.position=c(0.25,0.9),
        legend.title = element_text(size = 12),
        legend.text = element_text(size=10),
...
```{r imputation_diff2, fig.height=5.5, fig.width=8, fig.align='center', dev='svg', echo=FALSE, warning=FALSE, message=FALSE}
ggplot(data = impute, aes(y = factor(imputation, levels = c("Impute All",
          "Impute Pred-Obs (Min All)",
          "Impute Pred-Obs (Min Decile)",
          "Impute Pred-Obs (Min School)",
          "Impute Pred-Obs (Min School Decile)",
          "No Imputation")), 
                          x = mean, color = factor(cluster), fill = factor(cluster))) +
  geom_point(pch = 21, size = 4, stroke = 1.2) +
  geom_segment(aes(y = "Impute All", yend = "Impute All", x = impute$mean[impute$cluster==1 & impute$imputation=="Impute All"],
                   xend = impute$mean[impute$cluster==2 & impute$imputation=="Impute All"]), color = "grey") +
  geom_segment(aes(y = "Impute Pred-Obs (Min All)", yend = "Impute Pred-Obs (Min All)", x = impute$mean[impute$cluster==1 & impute$imputation=="Impute Pred-Obs (Min All)"],
                   xend = impute$mean[impute$cluster==2 & impute$imputation=="Impute Pred-Obs (Min All)"]), color = "grey") +
  geom_segment(aes(y = "Impute Pred-Obs (Min Decile)", yend = "Impute Pred-Obs (Min Decile)", x = impute$mean[impute$cluster==1 & impute$imputation=="Impute Pred-Obs (Min Decile)"],
                   xend = impute$mean[impute$cluster==2 & impute$imputation=="Impute Pred-Obs (Min Decile)"]), color = "grey") +
  geom_segment(aes(y = "Impute Pred-Obs (Min School)", yend = "Impute Pred-Obs (Min School)", x = impute$mean[impute$cluster==1 & impute$imputation=="Impute Pred-Obs (Min School)"],
                   xend = impute$mean[impute$cluster==2 & impute$imputation=="Impute Pred-Obs (Min School)"]), color = "grey") +
  geom_segment(aes(y = "Impute Pred-Obs (Min School Decile)", yend = "Impute Pred-Obs (Min School Decile)", x = impute$mean[impute$cluster==1 & impute$imputation=="Impute Pred-Obs (Min School Decile)"],
                   xend = impute$mean[impute$cluster==2 & impute$imputation=="Impute Pred-Obs (Min School Decile)"]), color = "grey") +
  geom_segment(aes(y = "No Imputation", yend = "No Imputation", x = impute$mean[impute$cluster==1 & impute$imputation=="No Imputation"],
                   xend = impute$mean[impute$cluster==2 & impute$imputation=="No Imputation"]), color = "grey") +
...
- Importance of **.coolblue[data availability]**
