...
load("https://raw.githubusercontent.com/maibennett/presentations/main/content/presentations/Attendance/IngUC_20230810/data/df_wide_testv5.RData")
...
df_wide_test$cluster = res.nbclust$best.partition
df_wide_test$cluster = factor(df_wide_test$cluster, levels = c(1,2), labels=c(1,2))

g1 = ggplot(df_wide_test, aes(x = cluster, y = diff_4_D1)) + 
        geom_boxplot(aes(fill = cluster, color = cluster)) + 
  xlab("Cluster") +
  ylab("Diff. obs - predicted") + ggtitle("D1 attendance") +
...
g2 = ggplot(df_wide_test, aes(x = cluster, y = diff_4_D2)) + 
        geom_boxplot(aes(fill = cluster, color = cluster)) + 
  xlab("Cluster") +
  ylab("Diff. obs - predicted") + ggtitle("D2 attendance") +
  scale_color_manual(values = c(cols[3],cols[2])) +
  scale_fill_manual(values = c(alpha(cols[3],0.4),alpha(cols[2],0.4))) +
  #ggplot2::annotate("text", x = 10, y = 6, label = "Adj. R2 = 0.48", size = 6, colour = "dark grey", hjust=0,family="Fira Sans Condensed") +
  theme_ipsum_fsc(plot_title_face = "bold",plot_title_size = 18) + #plain
...
g3 = ggplot(df_wide_test, aes(x = cluster, y = diff_4_D9)) + 
                geom_boxplot(aes(fill = cluster, color = cluster)) + 
  xlab("Cluster") +
  ylab("Diff. obs - predicted") + ggtitle("D9 attendance") +
  scale_color_manual(values = c(cols[3],cols[2])) +
  scale_fill_manual(values = c(alpha(cols[3],0.4),alpha(cols[2],0.4))) +
  #ggplot2::annotate("text", x = 10, y = 6, label = "Adj. R2 = 0.48", size = 6, colour = "dark grey", hjust=0,family="Fira Sans Condensed") +
  theme_ipsum_fsc(plot_title_face = "bold",plot_title_size = 18) + #plain
...
g4 = ggplot(df_wide_test, aes(x = cluster, y = diff_4_D10)) + 
                geom_boxplot(aes(fill = cluster, color = cluster)) + 
  xlab("Cluster") +
  ylab("Diff. obs - predicted") + ggtitle("D10 attendance") +
  scale_color_manual(values = c(cols[3],cols[2])) +
  scale_fill_manual(values = c(alpha(cols[3],0.4),alpha(cols[2],0.4))) +
  #ggplot2::annotate("text", x = 10, y = 6, label = "Adj. R2 = 0.48", size = 6, colour = "dark grey", hjust=0,family="Fira Sans Condensed") +
  theme_ipsum_fsc(plot_title_face = "bold",plot_title_size = 18) + #plain
...
d_tab <- read.csv(paste0(dir_data2,"data/working/d_tab.csv"))
...
ggplot(aes(x = Avg..SIMCE.Math, color = factor(cluster), fill = factor(cluster))) +
  ...
  scale_fill_manual(name = "Cluster", labels = c("Similar","Lower att"),values = c(alpha(cols[2],0.2),alpha(cols[3],0.2))) +
...
How can we **.coolblue[impute missing scores]**?
  ...
  - **.coolblue[Scenario 2]**: Impute by decile only for the difference between predicted and observed attendance.
    - Imputed score: (a) overall min, (b) decile min
...
impute2$mean[impute2$imputation %in% c("Impute Pred-Obs (All)", "Impute Pred-Obs (Decile)")] <- NA
