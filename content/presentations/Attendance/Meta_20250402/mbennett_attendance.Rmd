...
- Schools directly **.coolblue[(dis)incentivize attendance of (lower)higher performers]**
...
- .grey[Schools directly (dis)incentivize attendance of (lower)higher performers]
...
- Use **.coolblue[GPBoost (Extreme Gradient Boosting + Gaussian Process)]** with panel data for **.coolblue[attendance prediction]**
  ...
  - Predictor variables include lags for attendance, day of the week, grade, GPA group, in addition to student and schools' characteristics. 
...
g11 = g11 + theme(plot.margin = margin(0, 0, 0, 0)) +
  annotate("text", x = 5, y = 60, label = "RMSE = 0.051", color = "darkgrey", size = 3)
...
df_plot$ci_h = df_plot$ci_h*1.05
df_plot$ci_l = df_plot$ci_l*0.95

df_plot$ci_h[df_plot$ci_h>1] = 1

df_plot$outcome[df_plot$rbd==25002] = NA
df_plot$ci_h[df_plot$rbd==25002] = NA
df_plot$ci_l[df_plot$rbd==25002] = NA
...
ggplot(data = df_plot, aes(x = GPA_pos, y = outcome*100, fill = type, color = type)) + 
  ...
  geom_errorbar(aes(x=GPA_pos, ymin=ci_l*100, ymax=ci_h*100), width=0.1, alpha=0.9, size=0.8,
                position = position_dodge(width = 0.9))+
  scale_y_continuous(limits=c(0,100),oob = rescale_none) +
  theme_bw()+
  xlab("GPA deciles") + ylab("% Attendance") +
  ...
  theme(axis.title.x = element_text(size=14),#margin = margin(t = 10, r = 0, b = 0, l = 0)),
        ...
        title = element_text(size=14)) +
  facet_grid(grade_treat ~ factor(rbd, levels = c(10658,25002), labels = c("School 1", ""))) +
    theme(strip.text = element_text(family = "Fira Sans Condensed", size = 14))
```
...
```{r prediction_exampleb, fig.height=5, fig.width=10, fig.align='center', dev='svg', echo=FALSE, warning=FALSE, message=FALSE}
#df_plot = read.csv("https://raw.githubusercontent.com/maibennett/presentations/main/content/presentations/Attendance/IngUC_20230810/data/plot_data_gpboost2017_updated_examplev5.csv")
...
ggplot(data = df_plot, aes(x = GPA_pos, y = outcome*100, fill = type, color = type)) + 
  ...
  geom_ellipse(data = ann_plot, aes(x0 = 1, y0 = 60 , a = 0.5, b = 40, angle = 0),
              inherit.aes = FALSE, color = alpha(cols[2],0.5), lwd=1.2)
```
...
- Non-representative patterns of absenteeism **.coolblue[beyond exclusion of low-performers]**
  ...
  - Identification of gaming schools (e.g. persistence)

--
<br>
<br>

- Results presented to the **.coolblue[Educational Quality Agency]** for feedback

  - Use of ML algorithm to increase monitoring in a targeted way.

--
<br>
<br>

- **.coolblue[Can we predict test scores]**?

  - Absent students might be substantially different from observed students (_out of sample prediction_).
  - Use of no-consequences period (2022) as a benchmark.
