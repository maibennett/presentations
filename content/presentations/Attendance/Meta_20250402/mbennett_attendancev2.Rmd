...
.left[.small[Meta DSS<br>April 2nd, 2025]]]
...
```{r event_study_plot2nd, fig.height=5.5, fig.width=9, fig.align='center', echo=FALSE, warning=FALSE, message=FALSE}
library(ggplot2)
library(tidyverse)
library(hrbrthemes)
library(firasans)
...
```{r event_study_plot10th, fig.height=5.5, fig.width=9, fig.align='center', echo=FALSE, warning=FALSE, message=FALSE}
g10
```
...
```{r event_study_plot, fig.height=5.5, fig.width=10, fig.align='center', echo=FALSE, warning=FALSE, message=FALSE}
library(lemon)
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
# Convert #A5AC20 to RGBA for transparency
color_rgba <- "rgba(165, 172, 32, 0.3)"  # This includes the transparency
text_col <- "#A5AC20"  # Your specified color for the text

# Assuming tab4th is your data frame
# Apply cell_spec to the desired cells
tab4th$Told[1] <- cell_spec(tab4th$Told[1], "html", background = color_rgba, color = text_col, bold = TRUE)
tab4th$Notification[1] <- cell_spec(tab4th$Notification[1], "html", background = color_rgba, color = text_col, bold = TRUE)

# Now generate the table with kable and apply the additional styling
kable(tab4th, align = "lcccc", caption = "Results for 4th Grade",
      col.names = c('GPA Decile', 'Told', 'Notification', 'Preparation', 'Grades'), format = "html", escape = F) %>%
  kable_paper(bootstrap_options = c("hover", "condensed"), full_width = FALSE)
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
# Convert #A5AC20 to RGBA for transparency
color_rgba2 <- "rgba(68,147,174, 0.3)"  # This includes the transparency
text_color2 <- "#4493AE"  # Your specified color for the text

# Assuming tab4th is your data frame
# Apply cell_spec to the desired cells
tab4th$Told[3] <- cell_spec(tab4th$Told[3], "html", background = color_rgba2, color = text_color2, bold = TRUE)
tab4th$Notification[3] <- cell_spec(tab4th$Notification[3], "html", background = color_rgba2, color = text_color2, bold = TRUE)
...
tab10th$Told[1] <- cell_spec(tab10th$Told[1], "html", background = color_rgba, color = text_col, bold = TRUE)
tab10th$Notification[1] <- cell_spec(tab10th$Notification[1], "html", background = color_rgba, color = text_col, bold = TRUE)

tab10th$Told[3] <- cell_spec(tab10th$Told[3], "html", background = color_rgba2, color = text_color2, bold = TRUE)
tab10th$Notification[3] <- cell_spec(tab10th$Notification[3], "html", background = color_rgba2, color = text_color2, bold = TRUE)
...
```{r prediction_all_boxplot1, fig.height=5.3, fig.width=10, fig.align='center', echo=FALSE, warning=FALSE, message=FALSE}
...
```{r prediction_all_boxplot2, fig.height=5.3, fig.width=10, fig.align='center', echo=FALSE, warning=FALSE, message=FALSE}
g11 = g11 + xlab("")
...
--
```{r prediction_example, fig.height=5, fig.width=10, fig.align='center', echo=FALSE, warning=FALSE, message=FALSE}
...
```{r prediction_example2, fig.height=5, fig.width=10, fig.align='center', echo=FALSE, warning=FALSE, message=FALSE}
library(ggforce)
...
- **.coolblue[K-means clustering]** Use differences between predicted and observed attendance.
  - 2 optimal clusters]
--
```{r cluster_plot, fig.height=5, fig.width=8, fig.align='center', echo=FALSE, warning=FALSE, message=FALSE}
library(factoextra)
library(NbClust)
library(FactoMineR)
...
```{r loveplot1, fig.height=5, fig.width=10, fig.align='center', echo=FALSE, warning=FALSE, message=FALSE}
...
```{r loveplot2, fig.height=5, fig.width=10, fig.align='center', echo=FALSE, warning=FALSE, message=FALSE}
...
```{r loveplot3, fig.height=5, fig.width=10, fig.align='center', echo=FALSE, warning=FALSE, message=FALSE}
...
```{r loveplot4, fig.height=5, fig.width=10, fig.align='center', echo=FALSE, warning=FALSE, message=FALSE}
library(modelsummary)
library(designmatch)
...
ggplot(data = meanbal, aes(y = Variable, x = `Std Dif`)) +
  geom_point(size = 3, color = cols[2]) +
  geom_segment(aes(y = Variable, yend = Variable, x = ci_upper, xend = ci_lower), color = cols[2], linewidth = 0.8) +
  geom_vline(aes(xintercept = 0), lty = 2, linewidth = 1, color = "darkgrey") +
  ylab("") + xlab(expression(Delta* " Cluster 2 - Cluster 1 (SD)")) +
  ...
  theme(axis.title.x = element_text(size=14),#margin = margin(t = 10, r = 0, b = 0, l = 0)),
        ...
        axis.text.y = element_text(size=12),
        legend.position = "none") +
  annotate("rect", xmin = -1.5, xmax = 0.5, ymin = 1 - 0.45, ymax = 4 + 0.45, alpha = 0.1, fill = cols[2])
  

d_tab$cluster = factor(d_tab$cluster, levels = c(1,2), labels = c("Cluster 1\nIncrease att", "Cluster 2\nLower att (bottom)"))

#balance_tab = datasummary_balance(~ cluster, data = d_tab, dinm_statistic = "p.value", fmt = "%.2f", dinm = TRUE, align = c("lcccccc"),
#                    output = "data.frame",
#                    notes = "Note: Diff DX GPA represents the difference between obs. attendance and predicted attendance for decile X")

#balance_tab2 = balance_tab
#names(balance_tab2) = c(" ","Mean","SD","Mean","SD", "Diff. in Means", "P-value")

#kable(balance_tab2, format = "html", align = c("l", "c", "c", "c", "c", "c", "c")) %>%
#  kable_paper(bootstrap_options = c("hover", "condensed"), full_width = FALSE) %>%
#  add_header_above(c(" " = 1, "Cluster 1\nIncrease Att (N=1094)" = 2, "Cluster 2\nLower Att (bottom) (N=346)" = 2, " " = 2)) %>%
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
# Convert #A5AC20 to RGBA for transparency
color_rgba <- "rgba(148,66,181, 0.3)"  # This includes the transparency
text_col <- "#9442B5"  # Your specified color for the text

balance_tab = datasummary_balance(~ cluster, data = d_tab, dinm_statistic = "p.value", fmt = "%.2f", dinm = TRUE, align = c("lcccccc"),
                    output = "data.frame",
                    notes = "Note: Diff DX GPA represents the difference between obs. attendance and predicted attendance for decile X")

balance_tab$`Diff. in Means`[3] = cell_spec(balance_tab$`Diff. in Means`[3], "html", background = color_rgba, color = text_col, bold = TRUE)
balance_tab$`Diff. in Means`[5] = cell_spec(balance_tab$`Diff. in Means`[5], "html", background = color_rgba, color = text_col, bold = TRUE)
balance_tab$` `[3] = cell_spec(balance_tab$` `[3], "html", background = color_rgba, color = text_col, bold = TRUE)
balance_tab$` `[5] = cell_spec(balance_tab$` `[5], "html", background = color_rgba, color = text_col, bold = TRUE)

balance_tab2 = balance_tab
names(balance_tab2) = c(" ","Mean","SD","Mean","SD", "Diff. in Means", "P-value")

kable(balance_tab2, format = "html", align = c("l", "c", "c", "c", "c", "c", "c"), escape = F) %>%
  kable_paper(bootstrap_options = c("hover", "condensed"), full_width = FALSE) %>%
  add_header_above(c(" " = 1, "Cluster 1\nIncrease Att (N=1094)" = 2, "Cluster 2\nLower Att (bottom) (N=346)" = 2, " " = 2)) %>%
  kable_styling(bootstrap_options = c("hover", "condensed"), full_width = FALSE)
...
balance_tab$`Diff. in Means`[1] = cell_spec(balance_tab$`Diff. in Means`[1], "html", background = color_rgba, color = text_col, bold = TRUE)
balance_tab$`Diff. in Means`[2] = cell_spec(balance_tab$`Diff. in Means`[2], "html", background = color_rgba, color = text_col, bold = TRUE)
balance_tab$` `[1] = cell_spec(balance_tab$` `[1], "html", background = color_rgba, color = text_col, bold = TRUE)
balance_tab$` `[2] = cell_spec(balance_tab$` `[2], "html", background = color_rgba, color = text_col, bold = TRUE)
...
```{r imputation_diff1, fig.height=5.5, fig.width=10, fig.align='center', echo=FALSE, warning=FALSE, message=FALSE}
library(ggdist)
...
```{r imputation_diff2, fig.height=5.5, fig.width=10, fig.align='center', echo=FALSE, warning=FALSE, message=FALSE}
