...
text_color <- "#A5AC20"  # Your specified color for the text

# Assuming tab4th is your data frame
# Apply cell_spec to the desired cells
tab4th$Told[1] <- cell_spec(tab4th$Told[1], "html", background = color_rgba, color = text_color, bold = TRUE)
tab4th$Notification[1] <- cell_spec(tab4th$Notification[1], "html", background = color_rgba, color = text_color, bold = TRUE)
...
tab10th$Told[1] <- cell_spec(tab10th$Told[1], "html", background = color_rgba, color = text_color, bold = TRUE)
tab10th$Notification[1] <- cell_spec(tab10th$Notification[1], "html", background = color_rgba, color = text_color, bold = TRUE)
...
g1 = ggplot(df_wide_test, aes(x = cluster, y = diff_4_D1*100)) + 
  ...
  theme(axis.title.x = element_text(size=14),#margin = margin(t = 10, r = 0, b = 0, l = 0)),
        ...
        legend.text = element_text(size=15),
        legend.background = element_rect(fill="white",colour ="white"),
        plot.title = element_text(size=14, color = col_text))
...
text_color <- "#9442B5"  # Your specified color for the text

balance_tab = datasummary_balance(~ cluster, data = d_tab, dinm_statistic = "p.value", fmt = "%.2f", dinm = TRUE, align = c("lcccccc"),
                    output = "data.frame",
                    notes = "Note: Diff DX GPA represents the difference between obs. attendance and predicted attendance for decile X")

balance_tab$`Diff. in Means`[3] = cell_spec(balance_tab$`Diff. in Means`[1], "html", background = color_rgba, color = text_color, bold = TRUE)
balance_tab$`Diff. in Means`[5] = cell_spec(balance_tab$`Diff. in Means`[2], "html", background = color_rgba, color = text_color, bold = TRUE)
...
balance_tab$`Diff. in Means`[1] = cell_spec(balance_tab$`Diff. in Means`[3], "html", background = color_rgba, color = text_color, bold = TRUE)
balance_tab$`Diff. in Means`[2] = cell_spec(balance_tab$`Diff. in Means`[5], "html", background = color_rgba, color = text_color, bold = TRUE)
