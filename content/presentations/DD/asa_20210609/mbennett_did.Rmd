...
author: "ASA Seminar<br/>June 09, 2021"
output:
...
- For time-varying covariates:
  ...
  - Matching on pre-treatment covariates returns a biased ATT estimate **.darkorange[if covariates evolve differently over time and treatment does not affect them]**.

--

  - Matching on all covariates returns a biasded ATT estimate **.darkorange[if covariates evolve differently over time and are correlated to treatment]**.
...
Depending on the context, if we know the expected sign of the effect of $z$ on $x$, and the direction of the relationship of $x$ on $y$, then, e.g.:

$$\widehat{ATT} = [\hat{\tau}_{pre},\hat{\tau}_{all}]$$
...
- Adopt **.darkorange[flexibility]** when estimating effects (event study)]
.pull-right[
