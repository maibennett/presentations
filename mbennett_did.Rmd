...
Match on pre-treat outcomes          | `lm(y ~ a*p + t)` 
Match on pre-treat 1st diff          | `lm(y ~ a*p + t, data=out.match)` 
Match on pre-treat cov (PS)          | `lm(y ~ a*p + t, data=out.lag.match)` 
Match on pre-treat cov (MIP)         | `lm(y ~ a*p + t, data=out.match.mip)` 
]
