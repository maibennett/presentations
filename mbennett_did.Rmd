...
style_mono_accent(
  ...
  extra_css = list(
   ...
   # ".remark-slide table thead th" = list(),
    ...
    ".box-1,.box-1a,.section-title-1" = list("background-color" = "#0D0887"),
    ".box-2,.box-2a,.section-title-2" = list("background-color" = "#5601A4"),
    ".box-3,.box-3a,.section-title-3" = list("background-color" = "#900DA4"),
    ".box-4,.box-4a,.section-title-4" = list("background-color" = "#BF3984"),
    ".box-5,.box-5a,.section-title-5" = list("background-color" = "#E16462"),
    ".box-6,.box-6a,.section-title-6" = list("background-color" = "#F89441"),
    ".box-7,.box-7a,.section-title-7" = list("background-color" = "#FCCE25"),
...
<td id="highlight"><code class="remark-inline-code">lm(y ~ a*p + t, data=cov.match.mip)</code></td>
</tr>
<tr>
<td id="highlight">Match on all cov (MIP)</td>
<td id="highlight"><code class="remark-inline-code">lm(y ~ a*p + t, data=cov.match.mip.all)</code></td>
</tr>
</tbody>
</table>
</div>
...
```{r res1, fig.height=5, fig.align='center', dev='svg', echo=FALSE, warning=FALSE}

load("sim_resultsv3_match4_caliper0_nsim100.Rdata")

s1 = colMeans(m.s1[,-1], na.rm = TRUE)
s2 = colMeans(m.s2[,-1], na.rm = TRUE)
s3 = colMeans(m.s3[,-1], na.rm = TRUE)
s4 = colMeans(m.s4[,-1], na.rm = TRUE)
s5 = colMeans(m.s5[,-1], na.rm = TRUE)
s6 = colMeans(m.s6[,-1], na.rm = TRUE)
s5alt = colMeans(m.s5alt[,-1], na.rm = TRUE)

d = data.frame(cbind(c(s1,s2,s3,s4,s5,s5alt,s6),rep(c("Simple","CA","TVA","Match (level)",
                                                      "Match (trend)","Match (cov PS)",
                                                      "Match (cov MIP)",
                                                      "Match (cov MIP all)"),7),
                     c(rep("Time-invariant cov effect",8),
                       rep("Time-varying cov effect",8),
                       rep("Treat-independent cov effect",8),
                       rep("Parallel evolution",8),
                       rep("Evolution differs by group",8),
                       rep("Evolution differs by group (adj)",8),
                       rep("Evolution diverges in post",8)),
...
ggplot(d[d$var=="Constant",], aes(x = factor(adj, levels=unique(d$adj)), y = bias), fill="white") +
  ...
  theme(axis.title.x = element_text(size=16),#margin = margin(t = 10, r = 0, b = 0, l = 0)),
        ...
        legend.text = element_text(size=15),
        legend.background = element_rect(fill="white",colour ="white"),
        title = element_text(size=14)) + ylim(-0.5,1) +
  theme_xaringan(
    text_font = google_font("Fira Mono"),
    title_font = google_font("Fira Sans Condensed")
  ) +
  facet_grid(cols=vars(factor(type,levels = unique(d$type[d$var=="Constant"]))))
...
```{r res2, fig.height=5, fig.align='center', dev='svg', echo=FALSE, warning=FALSE}
ggplot(d[d$var=="Varying" & d$type!="Evolution differs by group",], 
  ...
  theme(axis.title.x = element_text(size=16, family="Fira Sans Condensed"),
        ...
        legend.text = element_text(size=15),
        legend.background = element_rect(fill="white",colour ="white"),
        title = element_text(size=14, family="Fira Sans Condensed")) + ylim(-0.5,1) +
  theme_xaringan(
    text_font = google_font("Fira Mono"),
    title_font = google_font("Fira Sans Condensed")
  ) +
